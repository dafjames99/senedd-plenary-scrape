...
An experiment is a YAML config (`services/embeddings/experiments/configs/*.yaml`) naming a model
...
3. **Measures** performance alongside quality: embed wall time and
   ...
   storage.
4. **Records** the run: one JSON object appended to `services/embeddings/experiments/runs.jsonl`
   (append-only machine log, committed to git) and a regenerated
   `services/embeddings/experiments/RESULTS.md` leaderboard (latest record per run id, ranked by
   MRR).
...
uv run python -m senedd_embeddings.experiments.runner --config services/embeddings/experiments/configs/baseline-gemma.yaml

# A candidate
uv run python -m senedd_embeddings.experiments.runner --config services/embeddings/experiments/configs/gemma-small-chunks.yaml

# Smoke-test a config end-to-end without embedding the whole corpus
# (flagged "partial" — quality numbers are not comparable)
uv run python -m senedd_embeddings.experiments.runner --config ... --limit 100

# Housekeeping
uv run python -m senedd_embeddings.experiments.runner --list                      # namespaces in DB
uv run python -m senedd_embeddings.experiments.runner --purge exp:name-a1b2c3d4   # drop one
uv run python -m senedd_embeddings.experiments.runner --config ... --no-keep      # purge on success
uv run python -m senedd_embeddings.experiments.runner --config ... --no-cache     # true provider timings
```
...
8. **Promote or purge.** Purge losing namespaces (`--purge`). To promote a
   ...
   - add/adjust its model entry in `MODEL_METADATA_REGISTRY`
     (`services/embeddings/senedd_embeddings/config.py`) — including `max_chunk_words` if the winning
