"""Run one embedding experiment end-to-end: embed → evaluate → record.
    ...
    uv run python -m senedd_embeddings.experiments.runner --config services/embeddings/experiments/configs/foo.yaml
    uv run python -m senedd_embeddings.experiments.runner --config ... --limit 200   # smoke test
    uv run python -m senedd_embeddings.experiments.runner --list                     # namespaces in DB
    uv run python -m senedd_embeddings.experiments.runner --purge exp:foo-a1b2c3d4   # drop vectors
...
or --purge later. See services/embeddings/experiments/README.md for the procedure this implements.
"""
...
from senedd_data.session import get_session  # noqa: E402
from senedd_data.settings import settings, setup_logging  # noqa: E402
from senedd_embeddings.providers import PROVIDER_REGISTER  # noqa: E402
from senedd_embeddings.experiments import embedder, results  # noqa: E402
from senedd_embeddings.experiments.config import ExperimentConfig, load_config  # noqa: E402
from senedd_embeddings.experiments.evaluation import evaluate, load_cases  # noqa: E402
...
def _print_report(record: dict) -> None:
    ...
    print("logged:  services/embeddings/experiments/runs.jsonl + services/embeddings/experiments/RESULTS.md")
    print("=" * 72)
