...
Every run appends one record to ``services/embeddings/experiments/runs.jsonl`` (append-only, one
JSON object per line — the machine-readable log) and rewrites
``services/embeddings/experiments/RESULTS.md`` (the human-readable leaderboard derived from the
