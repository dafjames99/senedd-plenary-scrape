...
- :mod:`src.db.provisioning` — schema/DDL lifecycle (Alembic + procedures).
- :mod:`src.db.acquisition`  — raw network + XML ingestion (source-of-truth tables).
- :mod:`src.db.transformation` — derived-table rebuild (no network).

New code should import those directly (and :func:`src.db.session.get_session`
...
from src.db.fetcher import Meeting
